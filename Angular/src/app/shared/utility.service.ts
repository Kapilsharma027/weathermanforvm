  ...
  handleErrorResponse(error, routeParameter?: string) {
    if (error.error != null) {
        if (error.error['code'] === 500) {
           ...
           const htmlContent = `<p>${error.error.message ? error.error.message : 'There is some error. Please check your network connectivity or try again later.'}</p>`;
            this.dialogService.errorDialogBox(htmlContent);
        }
       ...
       this.dialogService.errorDialogBox(htmlContent);
        ...
        // tslint:disable-next-line:max-line-length
        const htmlContent = `<p>${error.message ? error.message : 'There is some error. Please check your network connectivity or try again later.'}</p>`;
        this.dialogService.errorDialogBox(htmlContent);
    }
}
