  ...
  onSubmit(loginForm) {
    ...
    if (loginForm.valid) {
      ...
      this.loginService.login(this.loginrequestModel).subscribe(data => {
        ...
        this.storageService.write('userToken', data.data.token);
        this.storageService.write('userId', data.data.userId);
        ...
        Cookie.set("TheWeatherManToken", data.data.token, expire);
